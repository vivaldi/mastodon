...
class TranslationService::Lingvanex < TranslationService
  ...
  def translate(text, source_language, target_language)
    ...
    body = JSON.dump(q: text, source: modified_source_language.presence || source_language.presence || 'auto', target: target_language, translateMode: 'html')
  ...
  def fetch_languages
      request(:get, '/list/languages.json') do |res|
        JSON.parse(res.body_with_limit)
      end
  end
  ...
  def transform_response(json, source_language)
    data = JSON.parse(json, mode: :strict)
